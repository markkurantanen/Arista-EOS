...
op.add_option( '-c', '--service', dest='service', action='store', help='IP Address for Service VTEP leaf switch', type='string')
op.add_option( '-i', '--ingress', dest='intingress', action='store', help='Interface for connecting the FW interface 1', type='string')
op.add_option( '-e', '--egress', dest='integress', action='store', help='Interface for connecting the FW interface 2', type='string')
...
# extract parameters
device = opts.service
intIngress = opts.intingress
intEgress = opts.integress
vlan = opts.vlan
number = opts.number
...
# Create interfaces for FW ingress and egress
response = switch.runCmds(1, ["enable", "configure", "interface %s" % (intIngress), "no shutdown", "switchport mode trunk", "switchport trunk allowed vlan none", "spanning-tree portfast", "spanning-tree bpdufilter enable", "description fw-node-ingress"])
response = switch.runCmds(1, ["enable", "configure", "interface %s" % (intEgress), "no shutdown", "switchport mode trunk", "switchport trunk allowed vlan none", "spanning-tree portfast", "spanning-tree bpdufilter enable", "description fw-node-egress"])
